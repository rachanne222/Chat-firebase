...
function Login(props) {
	...
	const registerWithEmailAndPassword = async () => {
		console.log(auth);
		await createUserWithEmailAndPassword(auth, email.current.value, password.current.value).catch((error) => {
		...
		await logInWithEmailAndPassword(email, password).catch((error) => {
	...
	const logInWithEmailAndPassword = async () => {
		signInWithEmailAndPassword(auth, email.current.value, password.current.value)
			...
			.catch((error) => {
				...
				const errorMessage = error.message;
				var answer = window.confirm(
					{ errorMessage } + ' ' + 'Invalid email address or password do you need to register for Chat'
				);
				console.log(errorCode + ' ' + errorMessage);
				if (answer) {
